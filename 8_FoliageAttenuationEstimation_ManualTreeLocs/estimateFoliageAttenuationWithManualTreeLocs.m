...
% Generate a simpler version of this model comparison plot.
% Plot the models with the measurement results.
...
xlabel('Distance to TX (m)'); ylabel('Tree Number');
...
boolsTreeNsInFirstFresToShow = allTreeNumsInFirstFresnel>0; 
...
% Measurement results.
hExcePathLosses ...
    = scatter(allTreeNumsInFirstFresnel(boolsTreeNsInFirstFresToShow), ...
...
excePathLossMedian = arrayfun(@(n) ...
    ...
    treeNumValuesToShow);
g = fittype('a+x^b');
modExpPoly = fit(allTreeNumsInFirstFresnel(boolsTreeNsInFirstFresToShow), ...
