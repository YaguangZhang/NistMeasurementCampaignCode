...
% trees.
hConstShiftedForTreesFreeSpace ...
    = semilogx(sortedLosDist, ...
...
hCurLegend = legend([hMeas, hFreeSpace, ...
    ...
    hConstShiftedForTreesFreeSpace, hConstShiftedFreeSpace], ...
    'Measurements', ...
...
numPtsToShow = length(allTreeNumsInFirstFresnel);
boolsPosiTreeNums = allTreeNumsInFirstFresnel>0;
posiTreeNumValues = treeNumValues(treeNumValues>0);
% Add an error bar to better show the distribution of the path loss
% measurement results.
excePathLossStD = arrayfun(@(n) ...
    std(allExceLossRefFreeSpace(allTreeNumsInFirstFresnel==n)), ...
    posiTreeNumValues);
boolsPosTreeNumValues = treeNumValues>0;
hErrorBar = errorbar(treeNumValues(boolsPosTreeNumValues), ...
    excePathLossMean(boolsPosTreeNumValues), ...
    excePathLossStD, 'LineStyle', 'none', ...
    'Color', [0.8500 0.3250 0.0980], 'LineWidth', 1);
% Measurement results.
hExcePathLosses ...
    = scatter(allTreeNumsInFirstFresnel(boolsPosiTreeNums), ...
    allExceLossRefFreeSpace(boolsPosiTreeNums), ...
    ones(sum(boolsPosiTreeNums), 1) .* 8, ...
    'MarkerFaceColor','b','MarkerEdgeColor','none',...
    'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2);
...
excePathLossMedian = arrayfun(@(n) ...
    ...
    posiTreeNumValues);
g = fittype('a+x^b');
modExpDecay = fit(allTreeNumsInFirstFresnel(boolsPosiTreeNums), ...
    allExceLossRefFreeSpace(boolsPosiTreeNums), g, ...
    'StartPoint', rand(1,2));
...
fittedLine = [[0; treeNumValues(end)], ...
    [0; treeNumValues(end)].*excePathLossPerTree];
% Also plot a flat line (a constant offset with slope 0).
meanOfAllExcePathLosses = mean(allExceLossRefFreeSpace(boolsPosiTreeNums));
% Plots.
curLineWidth = 1;

hExcePathLossMedians = plot(treeNumValues(boolsPosTreeNumValues), ...
    excePathLossMedian, 'xb--', ...
    'LineWidth', curLineWidth);

xx = linspace(0,max(treeNumValues),100);
hExcePathLossFittedCurve = plot(xx, modExpDecay(xx), 'k-');
set(hExcePathLossFittedCurve, 'LineWidth', curLineWidth);
...
legend([hExcePathLosses, hErrorBar, hExcePathLossMedians, ...
    ...
    'Modified exponential decay', 'Mean for all data', ...
    'Location','northeast');
if flagModelCompOnTop
    xticklabels({}); 
else
...
axis([curAxis(1) curAxis(2)+0.5  curAxis(3)-deltaY curAxis(4)+3.*deltaY]);
curAxis = axis;
...
excePathLossFreq = arrayfun(@(n) ...
    sum(allTreeNumsInFirstFresnel==n), ...
    posiTreeNumValues);
bar(posiTreeNumValues, excePathLossFreq);
...
for idxS = 1:numOfSeries   
    shiftedFsplModExp{idxS} = freeSpacePathLosses{idxS} ...
        + modExpDecay(numsOfTreesInFirstFresnel{idxS});
end
...
fprintf(fIdRmse, ['FSPL with a modified exponential decay, ', ...
    'different for each tree number group, ', dbLossFormatter, '\r'], ...
    rmseModExpDecVsMeas);
