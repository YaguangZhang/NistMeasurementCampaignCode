...
% Set this flag to be true to hide the figures while they are being
% generated.
FLAG_GEN_FIGS_SILENTLY = true;
% Set this flag to be true to skip the outlayer inspections for different
% models.
flagSkipFigForDifferentModels = true;
...
if ~flagSkipFigForDifferentModels
    try
...
%% Inspect Outlayers for Different Models
...
if ~flagSkipFigForDifferentModels
    ...
    for idxModel = 1:numModelsToInspect
        ...
        hFigErrorsOnMap = figure('Visible', ~FLAG_GEN_FIGS_SILENTLY);
        hold on;
        ...
        hRx = plot3(rxLons, rxLats, zeros(size(rxLats)), rxLocMarker, ...
            ...
            'MarkerSize', rxLocMarkerSize, ...
            'LineWidth', rxLocMarkerLineWidth);
        ...
        % Repeat for a better output result: Manually adjust the visible
        % area.
        axis(figAxisToSet);
        plot_google_map('MapType','satellite');
        ...
        hFigOutlayerErrorsOnMap ...
            = figure('Visible', ~FLAG_GEN_FIGS_SILENTLY);
        hold on;
        hTx = plot(TX_LON, TX_LAT, '^g', ...
            'MarkerFaceColor', 'none', ...
            'LineWidth', 1.5);
        ...
        % The command plot_google_map messes up the color legend of plot3k,
        % so we will have to fix it here.
        hCb = findall( allchild(hFigOutlayerErrorsOnMap), ...
            'type', 'colorbar');
        hCb.Ticks = linspace(1,length(colormap)+1,length(hCb.TickLabels));
        ...
        hCb.TickLabels = arrayfun(@(n) num2str(n-errorShift, '%.1f'), ...
            cellfun(@(l) str2num(l), hCb.TickLabels), ...
            'UniformOutput', false);
    ...
    warning('Skipped outlayer inspection for different models.')
end
