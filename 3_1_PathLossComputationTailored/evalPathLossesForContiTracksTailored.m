% EVALPATHLOSSESFORCONTITRACKSTAILORED Load the Conti track measurements
% and their cooresponding GPS logs form the NIST dataset, and generate for
% each track a path loss over track (on map) plot.
%
% This script is basded on:
...
% We will nedd the original path loss computation scripts.
...
ABS_PATH_TO_ANT_PAT_FILE = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', 'AntennaPattern', 'antennaPatternFlipped.mat');
...
% Any point with its distance to the TX closer than this will be discarded.
MIN_VALID_DISTANCE_M = 50; % In meters.
...
[contiPathLossesWithGpsInfo, contiPathLossesExtraInfoTxGains, ...
    ...
    contiPathLossesExtraInfoDistsM] ...
...
for idxContiOutFile = 1:numContiOutFiles
    ...
    [curContiTxGains, curContiRxGains, curContiDistM] ...
    ...
    if ~any(isnan([curTxInfoLog.txAz, curTxInfoLog.txEl, ...
        ...
        for idxPathLoss = 1:numPathLosses
            ...
            %        antPatAz, antPatEl)
            [txGain, ~, ~, distM] ...
                = computeAntGain(TX_LAT, TX_LON, TX_ALT+TX_HEIGHT_M, ...
            ...
            curContiDistM(idxPathLoss) = distM;
    ...
    % Only keep points not too close to the TX.
    boolsResultsToKeep = curContiDistM>=MIN_VALID_DISTANCE_M;    
    
    % Store the results.
    contiPathLossesWithGpsInfo{idxContiOutFile} ...
        = curContiPathLossesWithGpsInfo(boolsResultsToKeep, :);
    contiPathLossesExtraInfoTxGains{idxContiOutFile} ...
        = curContiTxGains(boolsResultsToKeep);
    contiPathLossesExtraInfoRxGains{idxContiOutFile} ...
        = curContiRxGains(boolsResultsToKeep);
    contiPathLossesExtraInfoOriAlts{idxContiOutFile} ...
        = curContiOriAlts(boolsResultsToKeep);
    contiPathLossesExtraInfoSampIndices{idxContiOutFile} ...
        = curOutFileSampleRanges(boolsResultsToKeep, :);
    contiPathLossesExtraInfoDistsM{idxContiOutFile} ...
        = curContiDistM(boolsResultsToKeep);
    absPathsContiOutFiles{idxContiOutFile} ...
        = curAbsPathOutFile;
end
...
contiPathLossesExtraInfo.contiSampIndices ...
    = contiPathLossesExtraInfoSampIndices;
contiPathLossesExtraInfo.contiDistsM = contiPathLossesExtraInfoDistsM;
