% MANUALLYLOCATEMORETREES Manually mark more trees on a map for the NIST
% data set.
%
...
addpath(fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'NistMeasurementCampaignCode', '7_ManuallyLocateTrees'));
...
ABS_PATH_TO_SAVE_PLOTS = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', 'ManuallyLocateMoreTrees');
...
if FLAG_GENERATE_OVERVIEW_PLOTS_ONLY
    ...
    if exist(ABS_PATH_TO_VEG_AREAS_META, 'file')==2
        ...
        axisToShow = [-105.27701930, -105.27483020,  ...
        ...
        lidarLonLatNormZToShowRightHalf ...
            = lidarLonLatNormZToShow( ...
            lidarLonLatNormZToShow(:,1)>centerLon ...
            & lidarLonLatNormZToShow(:,2)>centerLat, ...
            :);
        
        normalizedZRightHalf = lidarLonLatNormZToShowRightHalf(:,3);
        normalizedZRightHalf ...
            = normalizedZRightHalf-min(normalizedZRightHalf);
        normalizedZRightHalf ...
            = normalizedZRightHalf/max(normalizedZRightHalf);
        lidarLonLatNormZToShowRightHalfNorm ...
            = [lidarLonLatNormZToShowRightHalf(:,1:2) ...
            normalizedZRightHalf];
        [~, hAx, hCb] = plot3k(lidarLonLatNormZToShowRightHalfNorm, ...
            'Marker',{'.',1});
        grid on; xlabel('Longitude'); ylabel('Latitude');
        zlabel('Nomalized Z');
        % title('Foliage Areas and Tree Locations (with Normalized LiDAR
        % Values)');
        xticks([]); yticks([]);
        axis(axisToShow);
        plot_google_map('MapType', 'satellite');
        ...
        cbPosToSet = [curCbPosition(1), ...
            curCbPosition(2)+curCbPosition(4)/2, ...
