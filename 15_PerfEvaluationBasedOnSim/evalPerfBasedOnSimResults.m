...
% Reuse results from compareFoliageDepthBasedModels.m, especially the
% foliage area info for the measurement dataset.
...
hAreaOfInterest = plot( ...
    ...
    'FaceColor', areaOfInterestColor);
plot(simGrid.latLons(:, 2), ...
    simGrid.latLons(:,1), '.b', 'MarkerSize', 2.5, 'Color', darkBlue);
% Use a redundant dot to better show the dot marker in legend.
hGridPts = plot(0, 0, '.b', 'MarkerSize', 5, 'Color', darkBlue);
...
startingPtTwoStepLinearWrtFA ...
    = [ ...
    modelTwoStepLinearLossWrtFARefitInForest.constPerUoFACloseFitted, ...
    modelTwoStepLinearLossWrtFARefitInForest.constPerUoFAFarFitted, ...
    modelTwoStepLinearLossWrtFARefitInForest.boundaryFitted, ...
    modelTwoStepLinearLossWrtFARefitInForest.constShiftFitted];
fittedRes = nlinfit( ...
    simGridPredResults.foliageAreasInFirstFresnel...
    (boolsRxLocInForestStrict), ...
    groundTruthPlValues(boolsRxLocInForestStrict) ...
    -simGridPredResults.fsplInDb(boolsRxLocInForestStrict), ...
    fctToFit, startingPtTwoStepLinearWrtFA);
modelTwoStepLinearLossWrtFARefitInForest.constPerUoFACloseFitted ...
    = fittedRes(1);
modelTwoStepLinearLossWrtFARefitInForest.constPerUoFAFarFitted ...
    = fittedRes(2);
modelTwoStepLinearLossWrtFARefitInForest.boundaryFitted = fittedRes(3);
modelTwoStepLinearLossWrtFARefitInForest.constShiftFitted = fittedRes(4);

siteSpecificModelCPredictionsInDbRefitInForest ...
    = simGridPredResults.fsplInDb(boolsRxLocInForestStrict) ...
    + modelTwoStepLinearLossWrtFARefitInForest ...
    .excessLossFormula( ...
    simGridPredResults.foliageAreasInFirstFresnel...
...
[rmsesSscOldInFS, rmsesItuOldInFS, rmsesSscRefitInFS] ...
    = deal(nan(numOfCurXsInFS,1));
rmseFct = @(diff) sqrt(mean( diff.^2 ));
for idxXInFS = 1:numOfCurXsInFS
    ...
    boolsInFGridPtsInCurRange = ...
        (rxToTxDistsInM3d(boolsRxLocInForestStrict)>=curXRangeMin) ...
        & (rxToTxDistsInM3d(boolsRxLocInForestStrict)<=curXRangeMax);
    rmsesSscRefitInFS(idxXInFS) = rmseFct(sscVsSimZsRefitInForest( ...
        boolsInFGridPtsInCurRange));
end
...
hSscOld = plot(curXsInFS, rmsesSscOldInFS, 'rx', ...
    'MarkerSize', 5);
hItuOld = plot(curXsInFS, rmsesItuOldInFS, 'go', 'MarkerSize', 3);
hSscRefitInForest = plot(curXsInFS, rmsesSscRefitInFS, 'b.', ...
    'MarkerSize', 6);
hSscOld = plot(curXsInFS, rmsesSscOldInFS, 'rx', ...
    'MarkerSize', 5);
curAxis = axis;
...
latLonRangePolyshapeXs = simGrid.latLonRangePolyshape.Vertices(:,1);
latLonRangePolyshapeXs = [latLonRangePolyshapeXs; latLonRangePolyshapeXs(1)];
latLonRangePolyshapeYs = simGrid.latLonRangePolyshape.Vertices(:,2);
latLonRangePolyshapeYs = [latLonRangePolyshapeYs; latLonRangePolyshapeYs(1)];
hSimGridBound = plot3(latLonRangePolyshapeXs, latLonRangePolyshapeYs, ...
    max(matRxLonLatWithPathLoss(:,3)+1) ...
    .*ones(size(latLonRangePolyshapeXs)), ...
    'k-', 'LineWidth', 0.75);
% Forest edge.
hForestEdge = plot3( ...
    simGridPredResults.clearanceZoneLatLonBoundaryAroundTx(:,2), ...
...
% Overview of the results from site-specific model C - simulation results
% with zero foliage area. The viriable zeroFaRegionPlotMethod changes how
% the alphashape of the boundary for the zero foliage area zone is
% generated. With "raw", the simulation grid points will be used. With
% "finer", a higher-resolution grid will be constructed and used, instead.
zeroFaRegionPlotMethod = 'raw'; % 'raw' or 'finer'.
if strcmpi(zeroFaRegionPlotMethod, 'finer')
    % Create a finer grid to replace the old one for better visualization
    % with alpha shapes.
    visualGridRatio = 10;
    visualGridXs = (min(simGridXs)-simGrid.resolutionInM):...
        (simGrid.resolutionInM/visualGridRatio):...
        max(simGridXs)+simGrid.resolutionInM;
    visualGridYs = (min(simGridYs)-simGrid.resolutionInM):...
        (simGrid.resolutionInM/visualGridRatio):...
        max(simGridYs)+simGrid.resolutionInM;
    [visualGridXs, visualGridYs] = meshgrid(visualGridXs, visualGridYs);
    visualGridXs = visualGridXs(:);
    visualGridYs = visualGridYs(:);
    boolsVisualZeroFoliageArea = false(size(visualGridXs));
    
    boolsZeroFoliageArea = simGridPredResults.foliageAreasInFirstFresnel==0;
    for idxBoolZeroFA = 1:length(boolsZeroFoliageArea)
        if boolsZeroFoliageArea(idxBoolZeroFA)
            boolsVisualZeroFoliageArea( ...
                visualGridXs<=(simGridXs(idxBoolZeroFA)+simGrid.resolutionInM) & ...
                visualGridXs>=(simGridXs(idxBoolZeroFA)-simGrid.resolutionInM) & ...
                visualGridYs<=(simGridYs(idxBoolZeroFA)+simGrid.resolutionInM) & ...
                visualGridYs>=(simGridYs(idxBoolZeroFA)-simGrid.resolutionInM)) = true;
        end
    end
    
    zeroFoliageAreaXYShape = alphaShape( ...
        visualGridXs(boolsVisualZeroFoliageArea), ...
        visualGridYs(boolsVisualZeroFoliageArea));
    zeroFoliageAreaXYShape.Alpha = zeroFoliageAreaXYShape.Alpha/5;
    zeroFoliageAreaXYShape.RegionThreshold ...
        = area(zeroFoliageAreaXYShape)/2;
    zeroFoliageAreaLonLatShapeEdgePtInds ...
        = boundary(zeroFoliageAreaXYShape.Points(:,1), ...
        zeroFoliageAreaXYShape.Points(:,2), 1);
    
    [zeroFoliageAreaShapeEdgeLats, zeroFoliageAreaShapeEdgeLons] = utm2deg( ...
        zeroFoliageAreaXYShape.Points(zeroFoliageAreaLonLatShapeEdgePtInds, 1), ...
        zeroFoliageAreaXYShape.Points(zeroFoliageAreaLonLatShapeEdgePtInds, 2), ...
        repmat(simGrid.utmZone, [length(zeroFoliageAreaLonLatShapeEdgePtInds), 1]));
    zeroFoliageAreaLonLatShapeEdge = ...
        [zeroFoliageAreaShapeEdgeLons, zeroFoliageAreaShapeEdgeLats];
elseif strcmpi(zeroFaRegionPlotMethod, 'raw')
    boolsZeroFoliageArea = simGridPredResults.foliageAreasInFirstFresnel==0;
    zeroFoliageAreaLonLatShape = alphaShape( ...
        simGridLons(boolsZeroFoliageArea), simGridLats(boolsZeroFoliageArea));
    zeroFoliageAreaLonLatShape.Alpha = zeroFoliageAreaLonLatShape.Alpha/4.75;
    ...
    error('Unknow plot method for zero foliage area region!');
end
...
hZeroFoliageArea = plot3( ...
    zeroFoliageAreaLonLatShapeEdge(:,1), ...
    zeroFoliageAreaLonLatShapeEdge(:,2), ...
    max(matRxLonLatWithPathLoss(:,3)+1) ...
    .*ones(size(zeroFoliageAreaLonLatShapeEdge(:,1))), ...
    'k:', 'LineWidth', 1);
% extraAx = axes('position',get(gca,'position'),'visible','off');
...
saveEpsFigForPaper(hCurPLMap, pathToSaveCurFig);
saveas(hCurPLMap, [pathToSaveCurFig, '.svg']);

% Evaluate the performance of the refitted SS-C model with the measurement
% data.
...
allPredictionsSSCInDbRefitInForest ...
...
disp(['The RMSE of refitted SS-C relative to measurements is ', ...
    num2str(rmseFct(allPredictionsSSCInDbRefitInForest-allMeas)), ' dB.'])
