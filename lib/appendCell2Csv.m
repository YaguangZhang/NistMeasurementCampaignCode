function appendCell2Csv(fileName, cellArray, precision, ...
    separator, excelYear, decimal)
...
% Modified by Yaguang Zhang, Purdue University, 12/27/2018, for controlling
% output digit precision and displaying progress.

% Modified by Yaguang Zhang, Purdue University, 12/27/2018, for appending 
% data into an existing cvs file.

% Set this to be true to show the progress of writing data into the file.
FLAG_DEBUG = false;
...
for row = 1:nrows
    if FLAG_DEBUG && (mod(row, 1000)==0)
        disp([num2str(row), '/' num2str(nrows)]);
    end
...
function x = StringX(x)
    ...
    elseif isnumeric(x) && isrow(x)
        x = num2StrPrecisionForFloat(x, precision);
        if decimal ~= '.'
            x = strrep(x, '.', decimal);
        end
