...
hLegend = legend([hMeas, hFreeSpace, ...
...
hMeasColoredByTreeNum ...
    = figure('Unit', 'pixels');
% Enlarge the figure horizontally a little bit.
curFigPos = get(hMeasColoredByTreeNum, ...
    'Position');
curFigPos(3) = 640;
curFigPos(4) = 420;
set(hMeasColoredByTreeNum, ...
    'Position', curFigPos);
% Free-space path losses.
hFreeSpace = plot3(sortedLosDist, ...
    allFreeSpacePathLosses(indicesForSortedDist).*0, ...
    ones(size(sortedLosDist)), ...
    '-', 'Color', ones(1,3).*0.7, 'LineWidth', 1);
hold on;
% Measured path losses.
...
hMeas = plot3(sortedLosDist, ...
    allContiPathLossesWithGpsInfo(indicesForSortedDist,1) ...
    -allFreeSpacePathLosses(indicesForSortedDist,1), ...
    allNumsOfTreesInFirstFresnel(indicesForSortedDist,1), ...
    'ok', 'MarkerSize', measMarkerSize);
maxNumOfTreesInFirstFresnel = max(allNumsOfTreesInFirstFresnel);
[~, hPlot3kAxis, hPlot3kColorbar] = plot3k([sortedLosDist, ...
...
%     -100.*ones(size(sortedLosDist)), 'x', 'Color', ones(1,3).*0.6, ...
%      'MarkerSize', measMarkerSize);
hLegend = legend([hMeas, hFreeSpace], ... hTwoStepCLPerUnitOfFoliageDepth], ...
    'Measurements colored by tree number', ...
    'Free-space path loss (FSPL)', ...
    ... 'Site-specific model A-I', ...
    'Location', 'northwest');
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hMeasColoredByTreeNum, ...
    fullfile(pathToSavePaperFigs, ...
    ['4_1_0_', ...
    'MeasAndFreeSpaceLosses_Simplified_ColoredByTreeNum.eps']));
...
hWeissberge = plot(allFoliageDepths, allPredictedPathLossesMod, '.r', ...
    'MarkerSize', 6);
hLegend = legend([hMeas, hFreeSpace, hWeissberge, hTwoStepCon], ...
    'Measurements', ...
    'Baseline FSPL (Foliage ignored)', ...
    'Weissberger''s model', ...
    'Site-specific model A-I', ...
    'Location', 'southeast');
transparentizeCurLegends;
...
legPosToSet = [curLegendPos(1)+5, curLegendPos(2)-5, curLegendPos(3:4)];
...
hItuModForFoliageDepth = plot( ...
    ...
    '.r', ...
    'MarkerSize', 6);
hLegend = legend([hMeas, hFreeSpace, hTwoStepCon, ...
...
overallModelMaePerfTable = computeMaesForDistSegs( ...
    {[0, inf]}, allTxRxLosDists, allMeasPathLosses, ...
    {'FSPL', allFreeSpacePathLosses; ...
    'AF', allPredictionsConstLossPerTrunk; ...
    'ITU', allPredictedPathLossesItu; ...
    ...
    'C_LinearLossWrtFA', allPredictedPathLossesLinearLossWrtFA; ...
    'C_TwoStepLinearLossWrtFA', allPredictedPathLossesTwoStepLinearLossWrtFA});
fFullPathMae = fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
    'overallMae.txt');
writetable(overallModelMaePerfTable, fFullPathMae);
disp(overallModelMaePerfTable);

MAX_ABS_ERR_ALLOWED_IN_DB = 10;
[overallModelNumMissedPtsPerfTable, ~, ...
    overallModelAccuraciesPerfTable, ~] ...
    ...
    'C_LinearLossWrtFA', allPredictedPathLossesLinearLossWrtFA; ...
    'C_TwoStepLinearLossWrtFA', allPredictedPathLossesTwoStepLinearLossWrtFA}, ...
    MAX_ABS_ERR_ALLOWED_IN_DB);
fFullPathNumMissedPts = fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
    'overallNumMissedPts.txt');
writetable(overallModelNumMissedPtsPerfTable, fFullPathNumMissedPts);
disp(overallModelNumMissedPtsPerfTable);

fFullPathAccuracies = fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
    'overallAccuracies.txt');
writetable(overallModelAccuraciesPerfTable, fFullPathAccuracies);
disp(overallModelAccuraciesPerfTable);

%%
% Make a accuracy plot for different MAX_ABS_ERR_ALLOWED_IN_DB.
maxAbsErrAllowedInDbToTest = 0:1:80;
...
lineStyles = {'.-', '--s', '-.^', ':*', '-', '--', '-.', ':'};
lineColors = {ones(1,3).*0.7, 'y', 'r', 'c', 'g', 'b', 'm', 'k'};
numMaxAbs = length(maxAbsErrAllowedInDbToTest);
[numModelsToInspect, ~] = size(modelsToInspect);
% The accuracy for one model will be a row in this matrix.
allAccuracies = nan(numModelsToInspect, numMaxAbs);
for idxMaxAbsErr = 1:numMaxAbs
    curMaxAbsErrAllowedInDb = maxAbsErrAllowedInDbToTest(idxMaxAbsErr);
    [~, ~, ...
        ~, curAccuraciesCell] ...
        = computeNumsOfMissedPtsForDistSegs( ...
        {[0, inf]}, allTxRxLosDists, allMeasPathLosses, ...
        modelsToInspect, ...
        curMaxAbsErrAllowedInDb);
    allAccuracies(:,idxMaxAbsErr) = cell2mat(curAccuraciesCell(:,2));
end
% Plot the accuracies.
hAccuracy = figure; hold on;
for idxModel = 1:numModelsToInspect
    plot(maxAbsErrAllowedInDbToTest, allAccuracies(idxModel,:), ...
        lineStyles{idxModel}, ...
        'Color', lineColors{idxModel}, ...
        'LineWidth', 1, 'MarkerSize', 3);
end
grid on;
legend(modelsToInspect(:,1), 'Location', 'southeast');
xlabel('Maximum Absolute Error Allowed (dB)');
ylabel('Accuracy');

%% Plot the accuracy improvements over FSPL.
hAccuracyOverFspl = figure; hold on;
for idxModel = 2:numModelsToInspect
    plot(maxAbsErrAllowedInDbToTest, ...
        allAccuracies(idxModel,:)-allAccuracies(1,:), ...
        lineStyles{idxModel}, ...
        'Color', lineColors{idxModel}, ...
        'LineWidth', 1, 'MarkerSize', 3);
end
grid on;
legend(modelsToInspect(2:end,1), 'Location', 'northeast');
xlabel('Maximum Absolute Error Allowed (dB)');
ylabel({'Accuracy Improvement';'over FSPL'});
...
%% Repeat Plotting
% New plots with the excess path loss (instead of the overall path loss)
% for the vertical axis.
FLAG_SHOW_FSPL = false;

hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum ...
    = figure('Unit', 'pixels');
% Enlarge the figure horizontally a little bit.
curFigPos = get(hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum, ...
    'Position');
curFigPos(3) = 640;
curFigPos(4) = 420;
set(hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum, ...
    'Position', curFigPos);
% Free-space path losses.
if FLAG_SHOW_FSPL
    hFreeSpace = plot3(sortedLosDist, ...
...
saveas(hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum, ...
    fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
    'MeasAndFreeSpaceExceLossesAndShiftedOverLogDist_Simplified_ColoredByTreeNum.fig'));
saveas(hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum, ...
    fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
    'MeasAndFreeSpaceExceLossesAndShiftedOverLogDist_Simplified_ColoredByTreeNum.png'));

% Export an .eps copy for papers.
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hMeasAndFreeSpaceExceLossesLogDistSimpColoredByTreeNum, ...
    fullfile(pathToSavePaperFigs, ...
    ['4_1_1_', ...
    'MeasAndFreeSpaceExceLossesAndShiftedOverLogDist_Simplified_ColoredByTreeNum.eps']));

PERF_COMP_FIG_WIDTH_IN_PIXEL = 560;
PERF_COMP_FIG_HEIGHT_IN_PIXEL = 370;

perfCompFigPos = [10, 10, ...
    PERF_COMP_FIG_WIDTH_IN_PIXEL, PERF_COMP_FIG_HEIGHT_IN_PIXEL];
hItuExcePerformance = figure('Unit', 'pixel', 'Position', perfCompFigPos);
hold on;
[~, indicesForSortedDistInWoodLand] = sort(allLosDistsInWoodland);
if FLAG_SHOW_FSPL
    hFreeSpace = plot( ...
        allLosDistsInWoodland(indicesForSortedDistInWoodLand), ...
        allFreeSpacePathLosses(indicesForSortedDistInWoodLand).*0, '-', ...
        'Color', ones(1,3).*0.7, 'LineWidth', 1);
end
hMeas = plot(allLosDistsInWoodland, ...
    allMeasPathLosses-allFreeSpacePathLosses, 'ob', ...
    'MarkerSize', 3);
% We will keep using the fixed-boundary verison.
hTwoStepCLPerUnitOfFoliageDepth = ...
    plot(allLosDistsInWoodland, ...
    allPredictedPathLossesTwoStepCon - allFreeSpacePathLosses, ...
    'g*', 'MarkerSize', 4);
% hTwoStepLinearLossWrtFA = ...
%     plot(allLosDistsInWoodland,
%     allPredictedPathLossesTwoStepLinearLossWrtFA, ...
%      'g*', 'MarkerSize', 4);
hItu = plot(allLosDistsInWoodland, ...
    allPredictedPathLossesItu - allFreeSpacePathLosses, ...
    '.r', 'MarkerSize', 8);
if FLAG_SHOW_FSPL
    hLegend = legend([hMeas, hFreeSpace, hItu, ...
        hTwoStepCLPerUnitOfFoliageDepth ... % hTwoStepCLPerUnitOfFoliageDepth ...
        ], ...
        'Measurements', ...
        'FSPL', ...
        'ITU obstruction by woodland', ...
    ...
    legPosToSet = [curLegendPos(1)+5, curLegendPos(2)-5, curLegendPos(3:4)];
    set(hLegend, 'Position', legPosToSet);
    axis tight; ylim([-40 95]);
else
    hLegend = legend([hMeas, hItu, ...
        hTwoStepCLPerUnitOfFoliageDepth ... % hTwoStepCLPerUnitOfFoliageDepth ...
        ], ...
        'Measurements', ...
        'ITU obstruction by woodland', ...
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hItuExcePerformance, ...
    fullfile(pathToSavePaperFigs, ...
    ['7_', ...
    'ItuExcePerformance_MeasVsLengthOfPathInWoodland.eps']));

hWeissExcePerformance ...
    = figure('Unit', 'pixel', 'Position', perfCompFigPos); hold on;
[~, indicesForSortedFoliageDepth] = sort(allFoliageDepths);
% FSPL with measurements.
if FLAG_SHOW_FSPL
    hFreeSpace = plot( ...
        allFoliageDepths(indicesForSortedFoliageDepth), ...
        allFreeSpacePathLosses(indicesForSortedFoliageDepth).*0, 'x', ...
        'Color', ones(1,3).*0.7, 'MarkerSize', 4);
end
hMeas = plot(allFoliageDepths, ...
    allMeasPathLosses-allFreeSpacePathLosses, 'ob', ...
    'MarkerSize', 3);
% A-I: Two-step const losses per unit of foliage depth.
hTwoStepCon = plot(allFoliageDepths, ...
    allPredictedPathLossesTwoStepCon - allFreeSpacePathLosses, 'g+', ...
    'MarkerSize', 4);
alpha(0.5);
% Weissberger's model.
hWeissberge = plot(allFoliageDepths, ...
    allPredictedPathLossesMod - allFreeSpacePathLosses, '.r', ...
    'MarkerSize', 6);
if FLAG_SHOW_FSPL
    hLegend = legend([hMeas, hFreeSpace, hWeissberge, hTwoStepCon], ...
        'Measurements', ...
    ...
    hLegend = legend([hMeas, hWeissberge, hTwoStepCon], ...
        'Measurements', ...
        'Weissberger''s model', ...
        'Site-specific model A-I', ...
        'Location', 'southeast');
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hWeissExcePerformance, ...
    fullfile(pathToSavePaperFigs, ...
    ['8_1_', ...
    'WeissbergerExcePerformance_MeasVsFoliageDepth.eps']));

hSiteSpecificModelsExce ...
    = figure('Unit', 'pixel', 'Position', perfCompFigPos); hold on;
% FSPL with measurements.
if FLAG_SHOW_FSPL
    hFreeSpace = plot( ...
...
if FLAG_SHOW_FSPL
    ...
    hLegend = legend([hMeas, hTwoStepCon, ...
        hTwoStepCLPerUoFAndCLForDEV, hItuModForFoliageDepth, ...
        hTwoStepLinearWrtFA], ...
        'Measurements', ...
        'Site-specific model A-I', ...
    ...
    set(hLegend, 'Units', 'pixel');
    curLegendPos = get(hLegend, 'Position');
    legPosToSet = [curLegendPos(1)+11, curLegendPos(2), curLegendPos(3:4)];
    set(hLegend, 'Position', legPosToSet);
end
transparentizeCurLegends; axis tight;
grid on; grid minor; xlabel('Foliage Depth \it{d_f} \rm{(m)}');
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hSiteSpecificModelsExce, ...
    fullfile(pathToSavePaperFigs, ...
    ['8_2_', ...
    'SiteSpecModelsExce_MeasVsFoliageDepth.eps']));

%% Plots for Model C

hSiteSpecificModelC = figure; hold on;
% FSPL with measurements.
hFreeSpace = plot( ...
    allFoliageAreasInFirstFresnel(indicesForSortedFoliageDepth), ...
    allFreeSpacePathLosses(indicesForSortedFoliageDepth), 'x', ...
    'Color', ones(1,3).*0.7, 'MarkerSize', 4);
hMeas = plot(allFoliageAreasInFirstFresnel, ...
    allMeasPathLosses, 'ob', ...
    'MarkerSize', 3);
% C: Two step linear for foliage area.
hTwoStepLinearWrtFA = plot( ...
    allFoliageAreasInFirstFresnel, ...
    ...
    '*k', ...
    'MarkerSize', 4);
hLegend = legend([hMeas, hFreeSpace, hTwoStepLinearWrtFA], ...
    'Measurements', ...
    'Baseline FSPL (Foliage ignored)', ...
    'Site-specific model C', ...
    'Location', 'southeast');
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hSiteSpecificModelC, ...
    fullfile(pathToSavePaperFigs, ...
    ['10_', ...
    'SiteSpecModelC_MeasVsFoliageArea.eps']));

hSiteSpecificModelCExce = figure; hold on;
% FSPL with measurements.
if FLAG_SHOW_FSPL
    hFreeSpace = plot( ...
        allFoliageAreasInFirstFresnel(indicesForSortedFoliageDepth), ...
...
if FLAG_SHOW_FSPL
    ...
    hLegend = legend([hMeas, hTwoStepLinearWrtFA], ...
        'Measurements', ...
        'Site-specific model C', ...
        'Location', 'southeast');
end
...
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hSiteSpecificModelCExce, ...
    fullfile(pathToSavePaperFigs, ...
    ['10_', ...
    'SiteSpecModelCExce_MeasVsFoliageArea.eps']));

%% Add Plots for One-Tree and Two-Tree Cases

boolsZeroTree = allNumsOfTreesInFirstFresnel==0;
boolsOneTree = allNumsOfTreesInFirstFresnel==1;
boolsTwoTrees = allNumsOfTreesInFirstFresnel==2;

% Pathloss over TX-to-RX distance.
hPathLossWithDiffTreeNums = figure; hold on;
% FSPL with measurements.
hFreeSpace = plot( ...
    allTxRxLosDists(indicesForSortedFoliageDepth), ...
    allFreeSpacePathLosses(indicesForSortedFoliageDepth).*0, 'x', ...
    'Color', ones(1,3).*0.7, 'MarkerSize', 4);
hMeas = plot(allTxRxLosDists, ...
    allMeasPathLosses-allFreeSpacePathLosses, 'ob', ...
    'MarkerSize', 3);
% No tree.
[sortedXs, indicesSortedByX] = sort(allTxRxLosDists(boolsZeroTree));
hZeroTree = plot( ...
    sortedXs, ...
    allMeasPathLosses(indicesSortedByX) ...
    -allFreeSpacePathLosses(indicesSortedByX), ...
    '.k-', ...
    'MarkerSize', 7);
% One tree.
[sortedXs, indicesSortedByX] = sort(allTxRxLosDists(boolsOneTree));
hOneTree = plot( ...
    sortedXs, ...
    allMeasPathLosses(indicesSortedByX) ...
    -allFreeSpacePathLosses(indicesSortedByX), ...
    ...
    'MarkerSize', 4);
% Two trees.
[sortedXs, indicesSortedByX] = sort(allTxRxLosDists(boolsTwoTrees));
hTwoTrees = plot( ...
    sortedXs, ...
    allMeasPathLosses(indicesSortedByX) ...
    -allFreeSpacePathLosses(indicesSortedByX), ...
    '*g--', ...
    'MarkerSize', 4);
hLegend = legend([hMeas, hFreeSpace, hZeroTree, hOneTree, hTwoTrees], ...
    'Measurements', ...
    'Baseline FSPL', ...
    'Zero tree', ...
    'One tree', 'Two trees');
transparentizeCurLegends; axis tight;
grid on; grid minor; xlabel('Tx-to-Rx Distance (m)');
ylabel('Excess Path Loss (dB)');
% Extend the yaxis a little bit to avoid blockage by the legends.
curAxis = axis; curAxis(4) = 100;
axis(curAxis);

% Export an .eps copy for papers.
pathToSavePaperFigs = fullfile(ABS_PATH_TO_NIST_SHARED_FOLDER, ...
    'PostProcessingResults', '1_EpsFigs');
saveEpsFigForPaper(hPathLossWithDiffTreeNums, ...
    fullfile(pathToSavePaperFigs, ...
    ['11_', ...
    'PathLossWithDiffTreeNums.eps']));

%% Plots for the NRSM Presentation
% We will focus on the site-specific model C and compare models using
% TX-to-RX distance as the x axis.

hItuExcePerformance = figure; hold on;
[~, indicesForSortedDistInWoodLand] = sort(allLosDistsInWoodland);
hFreeSpace = plot( ...
    allLosDistsInWoodland(indicesForSortedDistInWoodLand), ...
    allFreeSpacePathLosses(indicesForSortedDistInWoodLand).*0, '-', ...
...
hModelC = ...
    plot(allLosDistsInWoodland, ...
    allPredictedPathLossesTwoStepLinearLossWrtFA - allFreeSpacePathLosses, ...
    'g*', 'MarkerSize', 4);
% hTwoStepLinearLossWrtFA = ...
%     plot(allLosDistsInWoodland,
%     allPredictedPathLossesTwoStepLinearLossWrtFA, ...
...
hItu = plot(allLosDistsInWoodland, ...
    allPredictedPathLossesItu - allFreeSpacePathLosses, ...
    '.r', 'MarkerSize', 8);
legend([hMeas, hFreeSpace, hItu, hModelC ... % hTwoStepCLPerUnitOfFoliageDepth ...
    ], ...
    'Measurements', ...
    'FSPL', ...
    'ITU obstruction by woodland', ...
    'Site-specific model C', ...
    'Location', 'southeast');
transparentizeCurLegends; axis tight; ylim([-50 95]);
grid on; grid minor; xlabel('Length of Path within Woodland \it{d_w} \rm{(m)}');
ylabel('Excess Path Loss (dB)');
...
hItuExcePerformance = figure; hold on;
[~, indicesForSortedTxRxLosDists] = sort(allTxRxLosDists);
hFreeSpace = plot( ...
    allTxRxLosDists(indicesForSortedTxRxLosDists), ...
    allFreeSpacePathLosses(indicesForSortedTxRxLosDists).*0, '-', ...
    'Color', ones(1,3).*0.7, 'LineWidth', 1);
hMeas = plot(allTxRxLosDists, ...
    allMeasPathLosses-allFreeSpacePathLosses, 'ob', ...
    'MarkerSize', 3);
...
legend([hMeas, hFreeSpace, hItu, hModelC ... % hTwoStepCLPerUnitOfFoliageDepth ...
    ...
    'ITU obstruction by woodland', ...
    'Site-specific model C', ...
    'Location', 'southeast'); %'northwest');
transparentizeCurLegends;

axis tight; ylim([-50 95]);
% set(gca, 'xscale','log');
...
grid on; grid minor;
xlabel('Distance to TX \rm{(m)}'); ylabel('Excess Path Loss (dB)');
...
saveEpsFigForPaper(hItuExcePerformance, ...
    fullfile(pathToSavePaperFigs, ...
    ['NRSM_', ...
    'ItuExcePerformance_MeasVsDistToTx.eps']));

hWeissExcePerformance = figure; hold on;
[~, indicesForSortedFoliageDepth] = sort(allFoliageDepths);
% FSPL with measurements.
hFreeSpace = plot( ...
    allFoliageDepths(indicesForSortedFoliageDepth), ...
...
hWeissberge = plot(allFoliageDepths, ...
    allPredictedPathLossesMod - allFreeSpacePathLosses, '.r', ...
    'MarkerSize', 6);
hLegend = legend([hMeas, hFreeSpace, hWeissberge, hModelC], ...
    'Measurements', ...
    'Baseline FSPL (Foliage ignored)', ...
    'Weissberger''s model', ...
    'Site-specific model C', ...
...
hFreeSpace = plot( ...
    ...
    'Color', ones(1,3).*0.7, 'LineWidth', 1);
hMeas = plot(allTxRxLosDists, ...
    allMeasPathLosses - allFreeSpacePathLosses, ...
    'ob', ...
    'MarkerSize', 3);
% C: Two-step const losses per unit of foliage depth.
hModelC = plot(allTxRxLosDists, ...
    allPredictedPathLossesTwoStepLinearLossWrtFA ...
    - allFreeSpacePathLosses, ...
    'g*', ...
    'MarkerSize', 4);
alpha(0.5);
% Weissberger's model.
hWeissberge = plot(allTxRxLosDists, ...
    allPredictedPathLossesMod-allFreeSpacePathLosses, '.r', ...
    'MarkerSize', 6);
hLegend = legend([hMeas, hFreeSpace, hWeissberge, hModelC], ...
    'Measurements', ...
...
saveEpsFigForPaper(hWeissPerformance, ...
    fullfile(pathToSavePaperFigs, ...
    ['NRSM_', ...
    'WeissbergerExcePerformance_MeasVsDistToTx.eps']));

%% Regional performance.
windowSizeDistToTxInM = 10;
numXPtsToPlot = 1000;
figureSize = [560 270.5]; % [x y] in pixel.

minIntD = floor(min(allTxRxLosDists));
maxIntD = ceil(max(allTxRxLosDists));

% Define all models of interest.
cellAllPredictsItuD = { ... 'FSPL', allFreeSpacePathLosses; ...
    'ITU obstruction by woodland', allPredictedPathLossesItu; ...
    'Site-specific model C', allPredictedPathLossesTwoStepLinearLossWrtFA};
[numOfModelsD, ~] = size(cellAllPredictsItuD);

% For ITU.
xPtsToPlot = linspace(...
    minIntD+windowSizeDistToTxInM/2, maxIntD-windowSizeDistToTxInM/2, ...
    numXPtsToPlot);
segmentsOfInterest = arrayfun(@(x) ...
    [x-windowSizeDistToTxInM/2, x+windowSizeDistToTxInM/2], ...
    xPtsToPlot, 'UniformOutput', false);
% Compute the RMSEs.
[~, regionalRmsesItuCell] = computeRmsesForDistSegs( ...
    segmentsOfInterest, ...
    allTxRxLosDists, allMeasPathLosses, ...
    cellAllPredictsItuD);
% Plot
hFigRegionalRmsesItu = figure; hold on;
curFigPos = get(hFigRegionalRmsesItu, 'Position');
set(hFigRegionalRmsesItu, 'Position', [curFigPos(1:2) figureSize]);
refRmsesToPlotItu = [regionalRmsesItuCell{1, 2:end}];
for idxModD = 2:numOfModelsD
    curRmsesToPlotItu = [regionalRmsesItuCell{idxModD, 2:end}];
    plot(xPtsToPlot, refRmsesToPlotItu-curRmsesToPlotItu, ...
        markersForPlottingWithMod{1+mod(idxModD, ...
        length(markersForPlottingWithMod))}, ...
        'LineWidth', lineWith);
end
grid on; grid minor;
curAxis = axis; maxAbsY = max(abs(curAxis(3:4)));
axis([xPtsToPlot(1) xPtsToPlot(end) -maxAbsY maxAbsY]);
legend(cellAllPredictsItuD(2:end, 1), ...
    'Location', 'south', 'AutoUpdate', 'off');
xlabel('Distance to TX (m)');
ylabel({'Regional RMSE (dB)'; ...
    'Improvement over ITU'});
% Manully highlight the xaxis.
hXAxis = plot([0 maxIntD], [0 0], 'k-');
uistack(hXAxis, 'bottom');
...
% For Weissberger.
cellAllPredictsWeisD = { ...
    'Weissberger''s model', allPredictedPathLossesMod; ...
    'Site-specific model C', allPredictedPathLossesTwoStepLinearLossWrtFA};
[numOfModelsD, ~] = size(cellAllPredictsWeisD);
...
set(hFigRegionalRmsesWeis, 'Position', [curFigPos(1:2) figureSize]);
refRmsesToPlotWeis = [regionalRmsesWeisCell{1, 2:end}];
for idxModDf = 2:numOfModelsD
    curRmsesToPlotWeis = [regionalRmsesWeisCell{idxModDf, 2:end}];
    plot(xPtsToPlot, refRmsesToPlotWeis-curRmsesToPlotWeis, ...
        markersForPlottingWithMod{1+mod(idxModDf, ...
        length(markersForPlottingWithMod))}, ...
        'LineWidth', lineWith);
end
...
curAxis = axis; maxAbsY = max(abs(curAxis(3:4)));
axis([xPtsToPlot(1) xPtsToPlot(end) -maxAbsY maxAbsY]);
legend(cellAllPredictsWeisD(2:end, 1), ...
    'Location', 'south', 'AutoUpdate', 'off');
xlabel('Distance to TX (m)');
ylabel({'Regional RMSE (dB)'; ...
    'Improvement over WMED'});
