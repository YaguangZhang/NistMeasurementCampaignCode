...
if exist(pathToSaveModelInfo, 'file')
    ...
    for idxTrack = 1:numTracks
        ...
        parfor idxSamp = 1:numSamps
            ...
            curNumPixelsFirstFresZoneArea = sum(curFirstFresZoneArea(:));
            curNumNonFoliagePixelsInFirstFresnelZone(idxSamp) = ...
                curNumPixelsFirstFresZoneArea ...
                - curNumFoliagePixelsInFirstFresnelZone(idxSamp);
...
% For the constant loss for each unit of foliage area in the first Fresnel
% zone model with a shift when no foliage is present.
...
% For the constant loss for each unit of foliage area in the first
% Fresnel plus constant loss for each unit of the non-foliage one.
modelRem.excessLossFormula = @(a_f, constPerUoFA, a_n, constPerUoNFA) ...
    a_f.*constPerUoFA ...
    + a_n.*constPerUoNFA;

% For the constant loss for each unit of foliage area in the first
% Fresnel plus constant loss for each log unit of the non-foliage one.
...
% For the two-step constant loss for each unit of foliage area in the first
% Fresnel zone model.
modelTwoStepCLPerUoFA.boundaryFitted = 0.09;
...
hTwoStepCLPerUnitOfFoliageDepth = ...
    plot(allLosDistsInWoodland, allPredictedPathLossesTwoStepCon, 'g*', ...
    'MarkerSize', 4);
hItu = plot(allLosDistsInWoodland, allPredictedPathLossesItu, '.r', ...
    'MarkerSize', 8);
legend([hMeas, hFreeSpace, hItu, hTwoStepCLPerUnitOfFoliageDepth], ...
    'Measurements', ...
    'FSPL', ...
    'ITU obstruction by woodland', ...
...
grid on; grid minor; xlabel('Length of Path within Woodland \it{d_w} (m)');
ylabel('Path Loss (dB)');
...
hFreeSpace = plot( ...
    ...
    allFreeSpacePathLosses(indicesForSortedFoliageDepth), '-', ...
    'Color', ones(1,3).*0.7, 'LineWidth', 1);
hMeas = plot(allFoliageDepths, allMeasPathLosses, 'ob', ...
    'MarkerSize', 3);
...
legend([hMeas, hFreeSpace, hWeissberge, hTwoStepCon], ...
    ...
    'Location', 'southeast');
transparentizeCurLegends;
grid on; grid minor; axis tight; xlabel('Foliage Depth \it{d_f} (m)');
ylabel('Path Loss (dB)');
...
% enough vegetation.
hTwoStepCLPerUoFAndCLForDEV = plot(allFoliageDepths, ...
    allPredictedPathLossesTwoStepCLPerUoFAndCLForDEV, 'kx', ...
    'MarkerSize', 4);
% B: ITU style.
hItuModForFoliageDepth = plot( ...
...
ituModelPerfTable = computeRmsesForDistSegs( ...
    ...
    'A', allPredictedPathLossesTwoStepCon; ...
    'A_1', allPredictedPathLossesTwoStepConFixedB; ...
    'A_2', allPredictedPathLossesTwoStepCLPerUoFAndCLForDEV; ...
...
weisModelPerfTable = computeRmsesForDistSegs( ...
    ...
    {'Weis', allPredictedPathLossesItu; ...
    'A_1', allPredictedPathLossesTwoStepCon; ...
    'A_2', allPredictedPathLossesTwoStepCLPerUoFAndCLForDEV; ...
    'B', allPredictedPathLossesItuModForFoliageDepth});
fFullPathRmse = fullfile(ABS_PATH_TO_SAVE_PLOTS, ...
