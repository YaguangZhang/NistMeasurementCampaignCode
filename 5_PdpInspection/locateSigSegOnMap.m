...
indicesRelavantGpsSamps = ...
    find(arrayfun(@(r) ~isempty(range_intersection(r, sampIndexRange)), ...
    contiPathLossesExtraInfo.contiSampIndices{idxTrack}));
...
hAllGps = plot(contiPathLossesWithGpsInfoMatrix(:,3), ...
    contiPathLossesWithGpsInfoMatrix(:,2), '.', 'Color', ones(1,3));
